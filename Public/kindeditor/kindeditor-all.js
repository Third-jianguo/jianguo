...
K.options = {
	...
	htmlTags : {
        ...
        video : ['id', 'class', 'src', 'width', 'type', 'loop', 'autoplay', 'quality', '.width', '.height', 'align', 'allowscriptaccess', 'controls', 'preload'],
...
KindEditor.plugin('clearhtml', function(K) {
	...
	self.clickToolbar(name, function() {
		...
		html = K.formatHtml(html, {
            ...
            video : ['src', 'width',  'type', 'loop', 'autoplay', 'quality', '.width', '.height', 'align', 'allowscriptaccess', 'controls', 'preload'],
	...
	self.plugin.media = {
		edit : function() {
			var html = [
				...
				//autostart
				'<div class="ke-dialog-row">',
				'<label for="keAutoplay">' + lang.autoplay + '</label>',
				'<input type="checkbox" id="keAutoplay" name="autoplay" value="" /> ',
				'</div>',
				'</div>'
			].join('');
			var dialog = self.createDialog({
				...
				yesBtn : {
					...
					click : function(e) {
                        ...
                        if(autoplayBox[0].checked){
                            var html = K.mediaImg(self.themesPath + 'common/blank.gif', {
                                src : url,
                                type : K.mediaType(url),
                                width : width,
                                height : height,
                                autoplay : autoplayBox[0].checked ? 'true' : 'false',
                                controls : 'controls',
                                preload : 'preload'
                            });
                        }else{
			...
			widthBox = K('[name="width"]', div),
			heightBox = K('[name="height"]', div),
			autoplayBox = K('[name="autoplay"]', div);
			urlBox.val('http://');
			...
			if (img) {
				...
				widthBox.val(K.removeUnit(img.css('width')) || attrs.width || 0);
				heightBox.val(K.removeUnit(img.css('height')) || attrs.height || 0);
				autoplayBox[0].checked = (attrs.autoplay === 'true');
			}
			urlBox[0].focus();
			urlBox[0].select();
		},
