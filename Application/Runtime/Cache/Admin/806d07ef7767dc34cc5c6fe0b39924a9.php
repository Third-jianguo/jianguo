...
<div>
    ...
    <form action="/index.php/Admin/Games/index" >
        ...
        <div class="mt-20">
            <table class="table table-border table-bordered table-bg table-hover table-sort">
                ...
                <?php if(is_array($list)): $k = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($k % 2 );++$k;?><tr class="text-c va-m">
                        ...
                        <td><?php if(($vo["status"]) == "0"): ?>开启<?php else: ?>关闭<?php endif; ?></td>
                        <td class="td-manage">
                            <?php if(is_array($ActBtns)): $i = 0; $__LIST__ = $ActBtns;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$btn): $mod = ($i % 2 );++$i; if(($btn["action"]) != "add"): ?><a href="javascript:;" title="<?php echo ($btn["name"]); ?>" data-id="<?php echo ($vo["id"]); ?>" data-name="<?php echo ($vo["name"]); ?>" class="<?php echo ($btn["action"]); ?>">
    ...
    $(document).ready(function () {
        ...
        $('.del').click(function () {
            ...
            var index = layer.msg('是否删除' + title + '? <br>删除后不可恢复，请谨慎操作！', {
                ...
                yes: function () {
                    layer.close(index);
                    $.post('/index.php/Admin/Games/del', {id: id, title:title}, function (data) {
                        if (data.status == 1) {
                            layer.msg(data.msg, {
        ...
        $('.result_img').click(function () {
            ...
            var index = layer.open({
                ...
                area: ['85vw', '85vh'],
                fix: false, //不固定
                content: '/index.php/Admin/Games/result_img/id/' + id,
                btn: [ '关闭'],
                yes: function () {
                    layer.close(index);
                }
