...
<div>
    ...
    <form action="/index.php/Admin/Online/index" >
        ...
        <div class="mt-20">
            <table class="table table-border table-bordered table-bg table-hover table-sort">
                ...
                <?php if(is_array($list)): $k = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($k % 2 );++$k;?><tr class="text-c va-m">
                        ...
                        <td><?php echo (date("Y-m-d H:s:i",$vo["logintime"])); ?></td>
                        <td class="td-manage">
                            <?php if(is_array($ActBtns)): $i = 0; $__LIST__ = $ActBtns;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$btn): $mod = ($i % 2 );++$i; if(($btn["action"]) != "add"): ?><a href="javascript:;" title="<?php echo ($btn["name"]); ?>" data-uid="<?php echo ($vo["uid"]); ?>" data-id="<?php echo ($vo["id"]); ?>" data-name="<?php echo ($vo["title"]); ?>" class="<?php echo ($btn["action"]); ?>">
    ...
    $(document).ready(function () {
        ...
        $('.add').click(function () {
            ...
            var index = layer.open({
                ...
                area: ['85vw', '85vh'],
                fix: false, //不固定
                content: '/index.php/Admin/Online/add/',
                btn: ['确定', '关闭'],
                yes: function () {
                    var form = layer.getChildFrame('form', index);
                    $.post('/index.php/Admin/Online/add', form.serialize(), function (data) {
                        if (data.status == 1) {
                            layer.msg(data.msg, {
                                time: 2000,
                                end: function () {
        ...
        $('.save').click(function () {
            ...
            var index = layer.open({
                ...
                area: ['85vw', '85vh'],
                fix: false, //不固定
                content: '/index.php/Admin/Online/save/id/' + id,
                btn: ['确定', '关闭'],
                yes: function () {
                    var form = layer.getChildFrame('form', index);
                    $.post('/index.php/Admin/Online/save', form.serialize(), function (data) {
                        if (data.status == 1) {
                            layer.msg(data.msg, {
                                time: 2000,
                                end: function () {
        ...
        $('.del').click(function () {
            ...
            var index = layer.msg('是否强制下线' + title + '? ', {
                icon: 3,//1 √; 2 ×; 3 ?; 4 stock; 5 难过;6 微笑 ;7 ！;
                time: 0,
                shade: [0.8, '#393D49'],
                btn: ['下线', '取消'],
                yes: function () {
                    console.log('js send logout');
                    window.parent.ws.send('{"type":"logout","client_id":"'+uid+'","content":"退出登录"}');

                    layer.close(index);
                    window.location.reload();
//
//                    $.post('/index.php/Admin/Online/del', {id: id, title:title}, function (data) {
//                        if (data.status == 1) {
//                            layer.msg(data.msg, {
...
//                    }, 'json');
        ...
        $('.disable_data').click(function () {
            var title = $(this).data('name');
            var id = $(this).data('id');
            $.post('/index.php/Admin/Online/disable_data', {id: id}, function (data) {
                if (data.status == 1) {
                    layer.msg(data.msg, {
                        time: 1000, end: function () {
                            window.location.reload();
        ...
        $('.view').click(function () {
            ...
            var index = layer.open({
                ...
                area: ['85vw', '85vh'],
                fix: false, //不固定
                content: '/index.php/Admin/Online/view/id/' + id,
                btn: [ '关闭'],
                yes: function () {
                    layer.close(index);
                }
